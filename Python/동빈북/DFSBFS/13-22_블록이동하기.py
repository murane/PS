    ...
    while q:
        ...
        for i in range(4):
            ...
            if di:
                ...
                if board[Nx][Ny] != 0 or board[Nx][Ny + 1] != 0: continue
                if h_visit[Nx][Ny]!=-1 and h_visit[x][y]+1>=h_visit[Nx][Ny]:continue
                h_visit[Nx][Ny]=h_visit[x][y]+1
                q.append((Nx,Ny,True))
            else:
                if not 0<=Nx<l-1 or not 0<=Ny<l: continue
                if board[Nx][Ny] != 0 or board[Nx+1][Ny]!=0: continue
                if v_visit[Nx][Ny]!=-1 and v_visit[x][y]+1>=v_visit[Nx][Ny]:continue
                v_visit[Nx][Ny]=v_visit[x][y]+1
                q.append((Nx,Ny,False))
        if di:#가로->세로
            if x!=0 and board[x-1][y]==board[x-1][y+1]==0:
                if v_visit[x-1][y]==-1 or h_visit[x][y]+1<v_visit[x-1][y]:
                    v_visit[x-1][y]=h_visit[x][y]+1
                    q.append((x-1,y,False))
                if v_visit[x-1][y+1]==-1 or h_visit[x][y]+1<v_visit[x-1][y+1]:
                    v_visit[x-1][y+1]=h_visit[x][y]+1
                    q.append((x-1,y+1,False))
            if x!=l-1 and board[x+1][y]==board[x+1][y+1]==0:
                if v_visit[x][y]==-1 or h_visit[x][y]+1<v_visit[x][y]:
                    v_visit[x][y]=h_visit[x][y]+1
                    q.append((x,y,False))
                if v_visit[x][y+1]==-1 or h_visit[x][y]+1<v_visit[x][y+1]:
                    v_visit[x][y+1]=h_visit[x][y]+1
                    q.append((x,y+1,False))
        else:#세로->가로
            if y!=0 and board[x][y-1]==board[x+1][y-1]==0:
                if h_visit[x][y-1]==-1 or v_visit[x][y]+1<h_visit[x][y-1]:
                    h_visit[x][y-1]=v_visit[x][y]+1
                    q.append((x,y-1,True))
                if h_visit[x+1][y-1]==-1 or v_visit[x][y]+1<h_visit[x+1][y-1]:
                    h_visit[x+1][y-1]=v_visit[x][y]+1
                    q.append((x+1,y-1,True))
            if y!=l-1 and board[x][y+1]==board[x+1][y+1]==0:
                if h_visit[x][y]==-1 or v_visit[x][y]+1<h_visit[x][y]:
                    h_visit[x][y]=v_visit[x][y]+1
                    q.append((x,y,True))
                if h_visit[x+1][y]==-1 or v_visit[x][y]+1<h_visit[x+1][y]:
